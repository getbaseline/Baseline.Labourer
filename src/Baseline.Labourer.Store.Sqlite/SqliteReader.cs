...
public class SqliteReader : BaseSqliteInteractor, IStoreReader
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        scheduledJobsQueryCommand.Parameters.Add(new SqliteParameter("Cutoff", before.ToUniversalTime()));

        var reader = scheduledJobsQueryCommand.ExecuteReader();
        while (reader.Read())
        {
            results.Add(new ScheduledJobDefinition
            {
                Name = reader.GetString(0),
                CronExpression = reader.GetString(1),
                LastCompletedDate = reader.GetDateTime(2),
                LastRunDate = reader.GetDateTime(3),
