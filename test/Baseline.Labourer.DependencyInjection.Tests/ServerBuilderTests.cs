    ...
    public class ServerBuilderTests : BaseDependencyInjectionTest
    {
        ...
        public class ServerBuilderMiddleware : JobMiddleware
        {
            ...
            public override ValueTask JobStartedAsync(JobContext jobContext, CancellationToken cancellationToken)
            {
                ...
                Success = sc.JobProcessingWorkersToRun == 5 &&
                          ...
                          sc.JobRetryConfigurations[typeof(ServerBuilderJob)].Retries == RetryConfiguration.Default.Retries &&
                          sc.JobRetryConfigurations[typeof(ServerBuilderJob)].Delays.All(d => d == TimeSpan.FromSeconds((30)));
        ...
        {
            // Arrange.
            ConfigureServices((_, builder) =>
            {
                builder.UseMemoryQueue();
                builder.UseMemoryStore();
                builder.ConfigureServer(serverBuilder =>
                {
                    serverBuilder.RunThisManyJobProcessingWorkers(5);
                    serverBuilder.WaitThisLongBetweenCheckingForScheduledJobs(TimeSpan.FromSeconds(30));
                    serverBuilder.SetRetryConfigurationForAllJobsToBe(new RetryConfiguration(5, TimeSpan.Zero));
                    serverBuilder.AddRetryConfigurationForJobType<ServerBuilderJob>(RetryConfiguration.Default);
