...
public class SimpleScheduledJobWorkerTests : ServerTest
{
    ...
    public SimpleScheduledJobWorkerTests(ITestOutputHelper testOutputHelper)
    ...
    public async Task It_Processes_A_Scheduled_Task_That_Is_Past_Due_To_Run_And_Updates_The_Next_Run_Date()
    {
        ...
        TestStoreDataContainer.ScheduledJobs[scheduledJobId].NextRunDate = DateTime.UtcNow.AddHours(
            -1
        ...
        await AssertionUtils.RetryAsync(
            () =>
            {
                ...
                scheduledJob.NextRunDate
                    .Should()
                    .BeCloseTo(nextRunShouldBe, TimeSpan.FromMinutes(1));
            }
        );
    }
...
#pragma warning restore CS4014
        ...
        await AssertionUtils.RetryAsync(
            () =>
            {
                ...
                TestStoreDataContainer.ScheduledJobs[scheduledJobId].NextRunDate
                    .Should()
                    .Be(nextRunShouldBe);
            }
        );
    ...
    public async Task It_Copes_With_But_Does_Not_Schedule_A_Job_That_Already_Has_A_Lock_Present()
    {
        ...
        TestStoreDataContainer.Locks[scheduledJobId].Add(
            new MemoryLock { Until = DateTime.UtcNow.AddDays(7) }
        );
        ...
        await AssertionUtils.RetryAsync(
            () =>
            {
                TestStoreDataContainer.ScheduledJobs[scheduledJobId].NextRunDate
                    .Should()
                    .Be(DateTime.UtcNow.AddDays(1).Date);
